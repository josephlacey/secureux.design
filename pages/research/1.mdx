...
export default function Research1({ children }) {
  return (
    <MDXProvider components={uac().components(colors.orange)}>
      {children}
    </MDXProvider>
  );
}
...
> The condition of sharing or having certain attitudes and interests in common. {#community}

> Investigate (someone) thoroughly, especially in order to ensure that they are suitable for a job requiring secrecy, loyalty, or trustworthiness. {#vetted}
...
If you are not from the [community](#community) you are working with, it’s important to
...
any surveys or other communications you are creating for them are [vetted](#vetted), translated, adapted, and disseminated by
